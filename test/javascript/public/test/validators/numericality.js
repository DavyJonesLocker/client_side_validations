...
test('when value is a number', function() {
  ...
  element.val('123');
  equal(clientSideValidations.validator.numericality(validator, element), undefined);
});
...
test('when value is not a number', function() {
  ...
  element.val('abc123');
  equal(clientSideValidations.validator.numericality(validator, element), "failed validation");
});
