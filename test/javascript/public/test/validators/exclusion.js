...
test('when value is not in the list', function() {
  ...
  model.set({item: '4'});
  equal(ClientSideValidations.validators.local.exclusion(model, 'item', options), undefined);
});
...
test('when value is in the list', function() {
  var options = { 'message': "failed validation", 'in': [1, 2, 3] };
  model.set({item: '1'});
  equal(ClientSideValidations.validators.local.exclusion(model, 'item', options), "failed validation");
});
