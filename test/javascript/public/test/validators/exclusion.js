...
test('when value is not in the list', function() {
  ...
  selector.val('4');
  equal(clientSideValidations.validator.exclusion(validator, selector), undefined);
});
...
test('when value is in the list', function() {
  ...
  var validator = { message: "failed validation", in: [1, 2, 3] };
  selector.val('1');
  equal(clientSideValidations.validator.exclusion(validator, selector), "failed validation");
});
