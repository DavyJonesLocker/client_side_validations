...
test('when allowed length is 3 and value length is 3', function() {
  ...
  element.val('123');
  equal(clientSideValidations.validators.length(validator, element), undefined);
});
...
test('when allowed length is 3 and value length is 4', function() {
  ...
  var validator = { messages: { is: "failed validation" }, is: 3 };
  element.val('1234');
  equal(clientSideValidations.validators.length(validator, element), "failed validation");
});
