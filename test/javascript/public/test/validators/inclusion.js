...
test('when value is in the list', function() {
  ...
  element.val('1');
  equal(clientSideValidations.validators.inclusion(validator, element), undefined);
});
...
test('when value is not in the list', function() {
  ...
  var validator = { message: "failed validation", in: [1, 2, 3] };
  element.val('4');
  equal(clientSideValidations.validators.inclusion(validator, element), "failed validation");
});
