...
var clientSideValidations = new function() {
  ...
  this.validateSelector = function(selector, selectionStart, selectionEnd) {
    ...
    var validators = $.parseJSON(selector.attr('data-validators'));
  ...
  this.validator = {
    ...
    numericality: function(validator, selector) {
      ...
      for (var check in CHECKS) {
        if (validator[check] && !(new Function(selector.val() + CHECKS[check] + validator[check])())) {
    ...
    length: function(validator, selector) {
      if (message = this.presence(validator, selector) && validator.allow_blank == true) {
        ...
        var tokenized_length = new Function("return (selector.val()." + tokenizer + " || '').length;")();

        for (var check in CHECKS) {
          if (validator[check] && !(new Function(tokenized_length + CHECKS[check] + validator[check])())) {
            return validator.messages[check];
          }
        }
      }
